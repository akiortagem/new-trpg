...
The app does not include a playable adventure or main character. See [AUTHORING.md](AUTHORING.md) for both formats and a complete miniature example. See [CHOICE_STATE_CONDITIONS.md](CHOICE_STATE_CONDITIONS.md) for the choice-availability state contract, [OPTIONAL_SCENE_TITLES.md](OPTIONAL_SCENE_TITLES.md) for the scene-title visibility contract, and [OPTIONAL_CHECK_TWISTS.md](OPTIONAL_CHECK_TWISTS.md) for per-check Success with a Twist behavior.
...
- Per-check Success with a Twist. Checks that author `twist` and `twistPreview` offer the existing failure-or-twist decision; checks that omit both resolve rolled failure directly to their authored Failure outcome.
...
| `optional-check-twists.js` | Backward-compatible validation and runtime handling for checks that omit Success with a Twist. |
...
| `OPTIONAL_CHECK_TWISTS.md` | Per-check Success with a Twist contract and authoring behavior. |
...
| `tests/optional-check-twists.test.js` | Optional Success with a Twist validation/runtime regression tests. |

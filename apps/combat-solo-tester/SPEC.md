...
flowchart TD
    ...
    Tweaks --> Battle
    Battle --> Report
    Report -->|another queued encounter| Battle
    Report -->|session complete| Survey
    Survey --> Complete
    Complete --> Setup
```
...
2. Clones and applies PC tweaks.
3. Creates configured enemies.
...
`finishEncounter()` copies the completed encounter into `state.results`. Full-session mode then creates a fresh encounter from tweaks, resetting combat resources while retaining completed results and the final survey flow.

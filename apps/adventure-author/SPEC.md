...
Selecting a narrative scene opens a large modal editor so passages, choices, checks, conditions, effects, and outcomes have enough working space. Combat and ending nodes continue to use the right-hand inspector; battlefield editing has its own large modal. Editing is structured rather than raw JSON by default.

Entity ids are generated by the authoring app and are immutable in the structured editors. They remain in saved JSON as stable runtime references, but names, titles, and labels receive visual priority. Raw JSON remains available as the expert escape hatch.

A scene's authored `title` is also its graph label. Whether that title is shown to the player is controlled independently by the optional `showTitle` field; hiding a title in-game must never remove or replace the graph label.

### Scene editor

- title, with the generated id shown only as subdued metadata;
- **Show title in game** toggle;
- ordered passage list;
- passage list scrolls within its column while scene controls remain visible;
- Add Narration;
- Add Dialogue;
- adding a passage completes on the first click, preserves the passage editor context, and brings the new passage into view;
...
Dragging a graph node changes only its editor position. Releasing a node after a drag does not select or open it; a stationary click is required to open an editor.

Every newly created narrative scene begins with one automatic **Continue** choice. Authors may edit or remove it when the scene needs different branching.

### Scene title presentation

Ordinary scenes, combat scenes, and endings all expose the same title-presentation control.

`showTitle` is an optional boolean on the scene object. Omitted means `true`. When the toggle is on, the app may omit the property and rely on that default. Turning the toggle off writes `showTitle: false`.

`showTitle: false` suppresses the corresponding title presentation in the text game while preserving the authored `title` for the graph, destination labels, battlefield authoring, and other editor context. This allows branching scenes to continue naturally without sacrificing useful node names.

Existing titleless scenes remain supported. If `title` is omitted, `null`, or blank, there is no title to present regardless of `showTitle`.

### Choice inspector

Common:

- generated id shown only as subdued metadata;
- label;
- description;
